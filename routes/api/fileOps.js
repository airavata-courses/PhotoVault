...
// @route   GET api/:searchString
// @desc    Search an image based on searchString
// @access  Private
router.get('/:searchString', passport.authenticate('jwt', { session: false }), (req, res) => {
  ...
  res.json({ msg: req.params.searchString });
});
...
router.delete('/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
  ...
  User.findOne({ user: req.user.id }).then(MediaFile.findById(req.params.id)
    .then(post => {
      ...
      // Delete
      MediaFile.remove().then(() => res.json({
        success: true
      }));
    })
